...
void Transform2D::rotateSingleObject(Object2D *o, float angle) {
	
	Point2D *g = new Point2D();
	// Obtin centru de greutate obiect:
	g->x = (o->transf_points[0]->x + o->transf_points[1]->x + o->transf_points[2]->x + o->transf_points[3]->x) / 4;
	g->y = (o->transf_points[0]->y + o->transf_points[1]->y + o->transf_points[2]->y + o->transf_points[3]->y) / 4;

	// Translatez in origine:
	this->loadIdentityMatrix();
	this->translateMatrix(-g->x, -g->y);

	// Aplic rotatie:
	this->rotateMatrix(angle);

	// Translatez inapoi:
	this->translateMatrix(g->x, g->y);

	// Aplic transformari:
	this->applyTransform_o(o);
...
void Transform2D::applyTransformTextures(Object2D *o)
{
	//se aplica transformarea pe toate punctele obiectului
	for (int i = 0; i < 4; i++)
		applyTransform(o->texture_coords[i], o->texture_coords[i]);
	
}

void Transform2D::setTheNewG(Object2D *p, Object2D *o) 
{
	int j;
				for(j=0; j< o->transf_points.size(); j++)	
				{
					o->transf_points[j]->x = p->transf_points[j]->x;
					o->transf_points[j]->y = p->transf_points[j]->y;
