...
void DrawingWindow::drawObjects()
{
	...
	for (i = 0; i < texts.size(); i++)
	{
		...
		glRasterPos3f(texts[i]->pos.x, texts[i]->pos.y, 1);
...
//deseneaza obiecte 2D
	...
	for (i = 0; i < (int)objects2D.size(); i++)
	{
		...
		else if((objects2D[i]->type == 2) && (objects2D[i]->o_texture == true))
		{
			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, objects2D[i]->TexId);

			glBegin(GL_QUADS);
			glTexCoord2f( objects2D[i]->texture_coords[0]->x, objects2D[i]->texture_coords[0]->y ),		glVertex3f(objects2D[i]->transf_points[0]->x, objects2D[i]->transf_points[0]->y, 0);
			glTexCoord2f( objects2D[i]->texture_coords[1]->x, objects2D[i]->texture_coords[1]->y ),		glVertex3f(objects2D[i]->transf_points[1]->x, objects2D[i]->transf_points[1]->y, 0);
			glTexCoord2f( objects2D[i]->texture_coords[2]->x, objects2D[i]->texture_coords[2]->y ),		glVertex3f(objects2D[i]->transf_points[2]->x, objects2D[i]->transf_points[2]->y, 0);
			glTexCoord2f( objects2D[i]->texture_coords[3]->x, objects2D[i]->texture_coords[3]->y ),		glVertex3f(objects2D[i]->transf_points[3]->x, objects2D[i]->transf_points[3]->y, 0);
			glEnd();
		}
		else 
		{
...
void DrawingWindow::keyUpFunction(unsigned char key, int x, int y)
{
	onKeyUp(key);
}
